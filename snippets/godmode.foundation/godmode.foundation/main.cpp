...
#include "snippets/LLMChat_Snippet.h"

int main() {
    return AppRunner::init([]() {
        $_engines.add("GodmodeFoundation", function$((), new GodmodeFoundation()));
        $_apps.add("LLMChatSnippet", function$((), new LLMChatSnippet()));
    }).run();
}
