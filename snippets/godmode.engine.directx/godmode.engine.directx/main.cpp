...
int WINAPI WinMain(HINSTANCE app, HINSTANCE, LPSTR args, int opts) {
    return AppRunner::init(app, args, &opts, []() {
        godmode::$_engines.add("GodmodeEngineDirectX", function$((), new GodmodeEngineDirectX()));
        godmode::$_apps.add("EmptyAppSnippet", function$((), new EmptyAppSnippet()));
    }).run();
}
