...
import ollama from './source_$/ollama.js';
import config from './source_$/config.js';
...
app.post(config.llmController.promptRoute, (await import('./source_$/prompt.controller.js')).default);
app.listen(config.llmController.port, () =>
    console.log(`godmode.samples.LLMChat listening at http://localhost:${config.llmController.port}`));